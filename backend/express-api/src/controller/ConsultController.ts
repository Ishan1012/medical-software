...
export const consult = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if(!userId) {
            return res.status(403).json({
                suceess: false,
                message: "userId not found, please authenticate to use this service."
            });
        }

        const consult = await consultService.consult(symptoms, userId);
