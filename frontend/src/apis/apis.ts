...
export const signInApi = (signInRequest: SignInRequest) => api.post('/signin', signInRequest);

export const signUpApi = (signUpRequest: SignUpRequest) => api.post('/signup', signUpRequest);

export const signInByGoogleApi = (code: string, role: string) => api.post('/signin/google', { code, role }, { headers: { 'Content-Type': 'application/json' } });
