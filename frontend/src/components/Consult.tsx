...
const Consult = (): JSX.Element => {
  ...
  // Use the ConsultationResult interface defined above
  const [consultation, setConsultation] = useState<ConsultationResult | null>(null);
  
  // Use the Step interface defined above
  const steps: Step[] = [
  ...
  const updateFormData = <
    T extends 'personalInfo' | 'symptoms', 
    K extends keyof FormDataConsult[T]
  >(
    section: T,
    field: K,
    value: FormDataConsult[T][K]
  ) => {
    setFormData(prev => ({
      ...prev,
      [section]: {
